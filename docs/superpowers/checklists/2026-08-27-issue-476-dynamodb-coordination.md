...
- [x] **A-01 — 격리 및 요구사항 고정**
  ...
  - **Evidence:** `.worktrees/feat-issue-476-dynamodb-coordination`, branch `feat/issue-476-dynamodb-coordination`, base `origin/develop` (`da08bd12...`), issue #476 범위.
...
- [ ] **A-06 — test-first 구현**
  ...
  - **Evidence:** failing test, passing test, scoped diff.
  - **Failure:** P0/P1 또는 계약 위반은 구현 단계로 되돌린다.
- [ ] **A-07 — spec/plan/위험 검증**
  - **Action:** targeted/proportional tests와 repository hazards를 실행한다.
  - **Evidence:** verifier PASS, acceptance map, hazard results.
...
- [ ] **A-09 — lesson commit**
  ...
  - **Evidence:** lesson path, SPW-01..05, Lore commit.
...
- [x] **CG-01 — 권한 재확인**
  ...
  - **Evidence:** 현재 worktree status/read-back, spec/review/checklist path, base SHA `da08bd12f035e4a6824109c39b5bde3efabdb70e`.
...
완료 시 `Required checks: X/Y; N/A: 5; Blocked: 0` 형식으로 모든 row를 다시 집계한다. 현재 미검증 row는 `[ ]` 상태이며, PR/merge/cleanup은 각각 별도 gate다.
