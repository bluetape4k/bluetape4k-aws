...
| R-03 | emulator가 `DryRun` field를 무시해 실제 record를 남기거나 read operation을 정상 실행함 | operation별 disposable stream과 marker 비교를 사용한다. generic classifier는 정상 response를 실패로 유지하고, isolated scenario가 정상 response/marker persistence를 직접 확인한 경우에만 `dry_run_ignored_response`/`dry_run_ignored_write`라는 명시적 unsupported capability로 승격한다. | Task 4 네 isolated scenario, capability row, bounded marker read, Floci/LocalStack 순차 진단. | ignored 결과를 `SUPPORTED` 또는 no-write 증거로 사용하지 않는다. fake/model/public-client wire proof가 없으면 DONE/PR readiness를 선언하지 않는다. |
...
| R-05 | capability 분류가 인증·network·timeout·임의 정상 응답을 unsupported로 숨겨 CI가 거짓 GREEN이 됨 | generic classifier는 HTTP/error code closed set만 unsupported로 분류하고 정상 response를 `FAILED`로 유지한다. ignored 승격은 disposable stream의 operation-specific 정상 response와 write marker 관측에만 허용한다. upload는 test 성공과 validator 성공을 모두 요구한다. | Task 4 classifier negative test, isolated scenario, validator, workflow contract, PR CI/Full Nightly artifact read-back. | 누락·중복·unknown field/status, unexpected skip, closed-set 밖 오류가 있으면 job과 DoD를 실패/PENDING으로 유지한다. |
| R-06 | 실제 AWS endpoint, ambient credential, payload/header 또는 예외 message가 test log/artifact에 노출됨 | test-only client factory와 모든 resource/read helper가 endpoint allow-list와 static fake credential marker를 network 전 검사한다. bounded allow-list sanitizer와 client logging disable을 적용한다. validator 성공 파일만 업로드하며 Testcontainers credential property는 main/retry JUnit·HTML·log upload 전에 제거한다. retry binary/unknown extension은 업로드하지 않는다. | Task 3/4 negative endpoint/credential test, sentinel output capture, JSON validator, JUnit/retry sanitizer 회귀 테스트와 실제 artifact scan, 허용된 retry upload path를 고정한 두 workflow contract test. | sanitizer/validator 실패 또는 primary JUnit 누락 시 관련 artifact를 업로드하지 않는다. 실제 AWS 또는 default chain 경로는 즉시 중단한다. |
...
**현재 상태:** Task 4 local evidence에서 Floci `1.6.0`과 LocalStack `4`가 네 operation을
정상 실행했고 두 write marker를 저장함을 확인했다. 이 결과는 명시적 unsupported capability로
기록하며 AWS semantics 증거로 사용하지 않는다. compatibility, full build, exact-head CI는 PENDING이다.
