...
- [x] API/ABI, security, concurrency, caller/retry, operations, tests/docs/Kotlin
      여섯 plan review에서 P0=0, P1=0을 확인한다.
- [x] 사용자의 계획 승인 전에는 production/test code를 변경하지 않는다. 승인 후에도
      task별 RED를 먼저 실행한다.
- [x] worktree는 feat/issue-456-sns-batch, 기준은
...
- [x] 다음 dependencyInsight로 실제 SNS SDK version과 compileOnly 경계를 기록한다.
...
- [x] SnsClient.publishBatch, async future, response entry, builder parameter,
      CompletableFuture<PublishBatchResponse> return, and SDK member signature
      are output에서 확인한다.
- [x] aws-spring-boot/src/consumerFixture/kotlin/io/bluetape4k/aws/spring/sns/consumer/LegacySnsOperationsFixture.kt에
...
- [x] API 변경 전 baseline에서 다음 task를 실행하고 output class hash를
...
- [x] production interface가 변경된 뒤에는 legacy compile task를 다시 실행하지
      않는다. `aws-spring-boot/src/test/resources/sns-abi/`에 보존한 class resource를
      읽어 `LegacySnsOperationsFixture`를 별도 `ClassLoader`로 instantiate하고
      default batch dispatch와 preserved class hash를 검증한다.
- [x] production API는 이 task에서 바꾸지 않는다.

## Task 1: Java SDK v2 parity

### RED

- [x] aws-java/src/test/kotlin/io/bluetape4k/aws/sns/SnsClientTest.kt,
...
- [x] aws-java/src/main/kotlin/io/bluetape4k/aws/sns/model/Publish.kt에
      publishBatchRequestEntryOf와 publishBatchRequestOf를
      추가하고 attributes/FIFO/builder override를 보존한다.
- [x] exact declaration을 계획과 구현 diff에 고정한다:
...
- [x] aws-java/src/main/kotlin/io/bluetape4k/aws/sns/SnsClientExtensions.kt의
...
- [x] low-level Java/Kotlin API는 SDK exception과 future/await cancellation을
...
- [x] RED로 aws-kotlin/src/test/kotlin/io/bluetape4k/aws/kotlin/sns/SnsClientExtensionsTest.kt에
      Base58.randomString(16) ID/message를 사용한
      0/11/blank/duplicate 실패와 1/10 성공, topic ARN blank를 추가한다.
- [x] aws-kotlin/src/main/kotlin/io/bluetape4k/aws/kotlin/sns/SnsClientExtensions.kt의
...
- [x] RED로 aws-spring-boot/src/test/kotlin/io/bluetape4k/aws/spring/sns/SnsBatchModelsTest.kt와
...
- [x] aws-spring-boot/src/main/kotlin/io/bluetape4k/aws/spring/sns/SnsBatchModels.kt에
...
- [x] aws-spring-boot/src/main/kotlin/io/bluetape4k/aws/spring/sns/SnsBatchExceptions.kt에
...
- [x] RED로 legacy fixture와 NoopSnsOperations의 source/binary compatibility,
...
- [x] aws-spring-boot/src/main/kotlin/io/bluetape4k/aws/spring/sns/SnsOperations.kt에
      ...
      options를 1로 해석한다.
- [x] aws-spring-boot/src/test/kotlin/io/bluetape4k/aws/spring/sns/NoopSnsOperations.kt와
...
- [x] aws-spring-boot/src/test/kotlin/io/bluetape4k/aws/spring/sns/SnsBatchExecutorTest.kt에
      N=0,1,9,10,11,20,21,100 matrix와 exact chunk 수를 먼저 고정한다.
- [x] order 복원, mixed result, worker count 1/2/4,
      lazy iterator, resident entry 10 * maxInFlightBatches, sibling failure,
      collector/underlying cancellation, worker 종료, no-retry를 RED로 추가한다.
- [x] blocking publisher latency 자체는 timeout proof로 주장하지 않고 controlled fake의
      cancellation/cleanup만 관찰한다. high-volume 입력은 Base58.randomString(16)을
      사용하고 payload/ARN이 diagnostics에 나타나지 않는지 확인한다.
- [x] subject/attributes/FIFO mapping, standard/FIFO validation, protocol ID guard,
      low-level future cancellation, completedEntryIds와 failureType도 포함한다.
- [x] 다음 named race tests를 barrier-controlled fake publisher와 runTest
      virtual scheduler로 먼저 작성한다: claim 직후 caller cancellation,
      sibling transport failure 중 future cancellation, mixed-success 선행 chunk와
      sibling failure, 모든 경로의 worker 종료 및 pending future 0건. RED는 GREEN과 동일한 exact 명령으로
      구현 전 실행한다.
...
- [x] maxInFlightBatches가 0 또는 음수이면 즉시 거부하고, maxInFlightBatches가
      chunkCount보다 큰 경우 worker 수가 chunkCount를 넘지 않는다는 assertion을
      추가한다. rendezvous `Channel(capacity = 0)`과 ordered-flush `Semaphore`
      credit이 entry·worker·pending result resident bound를
      `10 * maxInFlightBatches` 안에 두는지 코드와 active counter로 검증한다.
      정상 경로 credit은 collector flush 시 반환하고, 취소 경로는 operation-local
      scope 종료로 정리한다.

## Task 6: 고정 worker executor GREEN

- [x] aws-spring-boot/src/main/kotlin/io/bluetape4k/aws/spring/sns/SnsBatchExecutor.kt를
      추가한다. lazy 10-entry chunk, empty fast path,
      sequential path, fixed workers, shared Mutex iterator claim을 사용한다.
- [x] SDK future await는 fixed worker 자체가 제한하는 외부 호출 경계에서 수행한다.
      별도 unbounded queue를 만들지 않으며 Mutex 안에서 SDK 호출·await·외부
      cancellation을 수행하지 않는다.
- [x] chunk sequence/entry ID로 order를 복원하고 completed IDs, protocol redaction,
...
- [x] README.md, README.ko.md, aws-spring-boot/README.md,
...
- [x] mixed-success 선행 chunk와 sibling transport failure가 함께 발생하면
...
- [x] #514/#515 링크와 범위를 문서에 반영하고 git diff --check와 bilingual
...
- [x] Floci smoke는 opt-in으로 -Dbluetape4k.aws.emulator=floci를 사용하고,
...
- [x] fixture compile/runtime, preserved class hash/javap ABI, redaction, cancellation, completed ID를
      최종 확인한다. SNS business rollback은 지원하지 않음을 별도로 확인한다.
      legacy fixture compile task는 Task 0 baseline 이후 다시 실행하지 않고,
      test-resource class/hash와 `SnsOperationsBatchCompatibilityTest`의 isolated
      `ClassLoader` 결과만 ABI 증거로 사용한다.
...
- [x] plan review P0=0/P1=0이고 P2는 #514/#515로 분리된다.
- [x] 계획 승인 후 명세·계획·review artifact를 커밋한다.
- [x] RED→GREEN exact command와 API, ABI, redaction, cancellation, bounded
