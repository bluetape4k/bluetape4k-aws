...
- [x] 설계·계획 파일을 추가하고 Korean terminology audit, `git diff --check`, SHA를 기록한다.
- [x] 보안·API·테스트·lifecycle·호환성·문서 관점의 독립 리뷰를 받아 design/plan review artifact에 통합한다.
- [x] Lore commit으로 문서 기준선을 기록한다.

### 1. catalog와 compileOnly 경계 (RED → GREEN)

- [x] consumer fixture에 raw `S3TablesClient` 참조를 먼저 추가해 alias 누락 RED를 확인한다.
- [x] `aws2.s3tables`, `aws.kotlin.s3tables` alias, 양 모듈 compileOnly/test dependency를 추가한다.
- [x] root consumer fixture classpath, BOM constraint, publication forbidden dependency를 추가한다.
- [x] 정상 compile(`./gradlew compileAwsJavaServiceConsumerFixture compileAwsKotlinServiceConsumerFixture`),
...
- [x] request helper: bucket/namespace/table create·get·list·delete와 최종 callback invariant.
- [x] `CreateTable.format=ICEBERG` 기본값·최종 검증, `GetTable` selector XOR, 한 페이지 목록 token/max/filter 계약을 테스트한다.
- [x] sync extension과 async extension/`.await()` delegation을 검증한다. 테스트 matrix는
...
- [x] application-scoped `s3TablesClient*`, short-lived `withS3Tables*Client`; close ownership을 고정한다.
- [x] MockK 기반 request mapping, async exception/cancellation, lifecycle 성공·예외·cancellation 테스트를 추가한다.

### 3. Kotlin API

- [x] Kotlin request helper와 native suspend extension을 추가한다.
- [x] `s3TablesClientOf`와 `withS3TablesClient`를 추가하고 `useSafe` lifecycle을 따른다.
- [x] Java와 대칭으로 `format=ICEBERG` 최종 검증, `GetTable` selector XOR, 한 페이지 token/max/filter와 callback override, exception/cancellation, caller-owned HTTP engine 테스트를 추가한다.

### 4. 문서와 opt-in smoke

- [x] root/module README EN·KO service matrix와 compile/runtime dependency 예제를 갱신한다.
- [x] Java/Kotlin manual에 관리 API 범위, Iceberg/Athena/Glue/Redshift 경계, lifecycle을 갱신한다.
- [x] CHANGELOG를 갱신한다. `WIP.md`는 오래된 backlog를 부분 수정하지 않고 제외 사실과 live GitHub 기준을 evidence에 기록한다.
- [x] read-only와 mutating real AWS smoke를 분리한다. mutating 경로는 명시 account/region, 고유 prefix, 이번 실행이 만든 리소스만 역순 삭제하는 `finally`, timeout, sanitized evidence를 요구한다.
- [x] mutating smoke는 첫 생성 전에 STS `GetCallerIdentity.account`와 `S3_TABLES_EXPECTED_ACCOUNT_ID`를 비교하고 불일치 시 fail-closed 한다. read-only는 `-Ps3TablesReadOnlySmoke` + `S3_TABLES_READ_ONLY_REGION` + `S3_TABLES_READ_ONLY_TABLE_BUCKET_ARN`, mutating은 `-Ps3TablesMutatingSmoke` + `S3_TABLES_EXPECTED_ACCOUNT_ID` + `S3_TABLES_MUTATING_REGION` + `S3_TABLES_MUTATING_PREFIX`를 계약으로 고정한다.
- [x] smoke 입력이 없으면 client 생성 전에 skip하고, 실행 태그는
...
- [x] targeted tests → full module tests → detekt/build를 순서대로 실행한다.
- [x] consumer fixture/publication, manual manifest/contract/inventory, `git diff --check`를 실행한다.
- [x] final code review와 evidence 문서를 exact HEAD 기준으로 작성한다.
...
- [x] 구현·테스트·문서 변경이 Issue #311의 bounded scope에 있다.
- [x] fresh local verification이 통과하고 결과가 evidence에 기록되어 있다.
- [ ] PR head/CI/review/mergeability를 재확인했으며 merge는 하지 않았다.
- [x] known gap(실제 AWS IAM/resource smoke, emulator fidelity 등)이 명시되어 있다.
