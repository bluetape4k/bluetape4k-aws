...
| `aws-spring-boot/src/test/kotlin/io/bluetape4k/aws/spring/sns/SnsPropertiesCompatibilityTest.kt` | 기존 `SnsProperties` JVM constructor/copy/default-arg descriptor reflection 검증 |
...
- [x] **Step 3: 최소 구현** — trim/blank/topic-name 검증, SNS ARN parser와 기본 cross-account 거부, account 미설정 explicit ARN fail-closed, `arn:` direct return, `ListTopics` nextToken loop와 `:$topicName` suffix 비교 및 반환 ARN scope 검증, 성공/null만 cache 저장을 구현한다. 예외와 `CancellationException`은 catch하지 않거나 재전파한다.
...
- [x] **Step 3: 최소 구현** — private primary constructor에 resolver를 추가하고 기존 public constructor를 보존한다. `SnsProperties`의 기존 5-인자 constructor/component/copy ABI도 additive property 배치와 compatibility overload로 유지한다. `findTopicArn`은 resolver에 위임하고 create 성공 ARN 반환 직전에 invalidate한다. batch/publish/SMS/confirmation 코드는 변경하지 않는다.
...
- [x] **Step 1: RED 테스트 작성** — 기본 cache/resolver bean, `enabled=false`의 `Noop` cache, custom cache/resolver back-off, properties와 `SnsConnectionDetails` 각각의 effective endpoint/region, account/cross-account policy binding, uninspectable custom client의 fail-fast와 custom resolver back-off를 확인한다.
...
## Task 7: Type-A verifier, review, lesson, PR delivery
...
- [x] **Step 5: delivery boundary** — Issue #474의 repository/base/head와 PR·merge·sync-cleanup 권한이 승인되어 Type-A delivery lane으로 전환한다. PR 생성, merge, canonical `develop` sync와 안전한 cleanup은 각각 별도 gate와 fresh evidence로 수행한다.

### Step 6-R 통합 review evidence

| 관점 | 초기 finding | 보완 및 현재 판정 |
|---|---|---|
| Performance | P1 3, P2 3 | transient outcome, Noop/LRU 동시성, positive hit/TTL, key별 병렬 테스트를 추가했다. P0=0, P1=0; active-flight 상한은 caller 수명과 map 제거로 bounded하므로 global semaphore를 도입하지 않는 P2 보류다. 후속 stress는 #564다. |
...
| Operations | P1 3, P2 1 | EN/KO README·manual에 기본값·rollback·custom bean 경로를 추가하고, terminal failure에 hash 기반 warning을 한 번만 기록하도록 했다. P0=0, P1=0. |
| Developer/API | P1 1, P2 2 | 기존 `SnsProperties` 5-인자 JVM constructor/component/copy descriptor를 보존하고, null account explicit ARN fail-closed 및 ListTopics ARN 검증을 추가했다. resolver/template API compatibility test를 확장했다. P0=0, P1=0. |
| User/Caller | P1 0, P2 0 | name/ARN 입력, account 설정 요구, cache disable과 full rollback 차이, eventual consistency와 관측 신호를 문서화했다. P0=0, P1=0. |
| Main integration | — | six-lens review, Korean KDoc/spec/plan/lesson, scope·ABI·emulator evidence와 exact current head를 재대조한다. CI receipt가 아직 없어 delivery-ready 판정은 PR exact-head checks 후 확정한다. |
...
| rollout/operability | lookup failure 급증 또는 잘못된 ARN 응답 | hashed warning, configured account/region validation, custom resolver/cache는 구성 override로 한정, custom `SnsOperations` 또는 last-known-good artifact로 behavior-preserving rollback, full SNS `enabled=false`는 전체 비활성화 | PR CI와 운영 로그 확인 후 좁은 rollback은 custom operations/이전 artifact, 전체 차단은 `enabled=false` |
