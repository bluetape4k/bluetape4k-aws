...
**Tech Stack:** Kotlin 2.x, kotlinx-coroutines `Mutex`, AWS SDK v2 `SnsAsyncClient`, Spring Boot 4 auto-configuration, JUnit 5, MockK, Floci.
...
| `aws-spring-boot/src/main/kotlin/io/bluetape4k/aws/spring/sns/SnsProperties.kt` | `accountId`, `topicArnCache` Spring Boot property binding과 validation |
...
- [ ] **Step 1: RED 테스트 작성** — `SnsTopicArnCache`의 TTL, LRU max size, negative entry, invalidate/clear와 `SnsProperties`의 `topicArnCache` binding을 호출할 기대 API로 작성한다. AWS client mock은 `ListTopicsResponse`의 전체 구조를 제공하며 mock 자체를 검증하지 않고 resolver 결과와 호출 횟수를 검증한다.
- [ ] **Step 2: RED 실행**
...
- [ ] **Step 3: 최소 cache/property 구현** — `DataKeyCache`의 `Clock`/LRU/TTL 패턴을 복사하되 SNS entry는 `Resolved`와 `NotFound`를 구분한다. `maxSize <= 0`, non-positive TTL, blank account ID를 즉시 거부한다.
- [ ] **Step 4: GREEN 실행** — 동일 명령을 다시 실행해 cache/property 테스트가 통과하는지 확인한다.
- [ ] **Step 5: checkpoint** — `git diff --check`와 테스트 출력에서 경고/실패를 확인하고, 실패 시 Task 1로 되돌아간다.
...
- [ ] **Step 1: RED 테스트 작성** — pagination 2페이지, topic 미존재 negative cache, explicit ARN 우회, `.fifo` suffix 정확성, AWS 예외 미저장 테스트를 추가한다.
- [ ] **Step 2: RED 실행** — Task 1 targeted command로 새 테스트가 계약 누락 때문에 실패하는지 확인한다.
- [ ] **Step 3: 최소 구현** — trim/blank 검증, `arn:` direct return, `ListTopics` nextToken loop와 `:$topicName` suffix 비교, 성공/null만 cache 저장을 구현한다. 예외와 `CancellationException`은 catch하지 않거나 재전파한다.
- [ ] **Step 4: GREEN 실행** — resolver 테스트를 단독 실행해 pagination/normalization/negative/error 경계가 통과하는지 확인한다.
...
- [ ] **Step 1: RED 테스트 작성** — 같은 name의 동시 coroutine이 SDK를 1회만 호출하는지, failure 후 다음 호출이 재시도하는지, caller cancellation 후 flight가 남지 않는지, 다른 endpoint/region/account scope가 같은 cache에서 분리되는지 검증한다.
- [ ] **Step 2: RED 실행** — `runTest`에서 unresolved `CompletableFuture`를 사용해 duplicate call count가 2가 되는 기대 실패를 확인한다.
- [ ] **Step 3: 최소 구현** — `Mutex`로 flight table 접근을 보호하고 key별 user count를 추적한다. 같은 key는 `Mutex.withLock`으로 double-check와 lookup을 직렬화하고, `finally`에서 user count가 0이면 flight를 제거한다. background scope/`GlobalScope`/`runBlocking`은 사용하지 않는다.
- [ ] **Step 4: GREEN 실행** — 동시성·취소 테스트를 단독 실행하고, cancellation identity가 swallowed되지 않았는지 확인한다.
...
- [ ] **Step 1: RED 테스트 작성** — injected resolver를 사용한 `findTopicArn` delegation, `createTopic`/`createFifoTopic` 성공 후 invalidate, 기존 2개 생성자 compile/behavior compatibility를 검증한다.
- [ ] **Step 2: RED 실행** — template test에서 injected resolver 생성자 또는 delegation이 없어 실패하는지 확인한다.
- [ ] **Step 3: 최소 구현** — private primary constructor에 resolver를 추가하고 기존 public constructor를 보존한다. `findTopicArn`은 resolver에 위임하고 create 성공 ARN 반환 직전에 invalidate한다. batch/publish/SMS/confirmation 코드는 변경하지 않는다.
- [ ] **Step 4: GREEN 실행** — `SnsCoroutinesTemplateTest`와 resolver 테스트를 함께 실행한다.
...
- [ ] **Step 1: RED 테스트 작성** — 기본 cache/resolver bean, `enabled=false`의 `Noop` cache, custom cache/resolver back-off, endpoint/region/account scope binding을 확인한다.
- [ ] **Step 2: RED 실행** — auto-configuration test에서 bean 누락/constructor mismatch failure를 확인한다.
- [ ] **Step 3: 최소 구현** — `@ConditionalOnMissingBean` cache/resolver bean을 등록하고 properties에서 scope/cache options를 전달한다. operations bean이 custom이면 기존처럼 template을 만들지 않는다.
- [ ] **Step 4: GREEN 실행** — auto-config 및 compatibility test를 실행한다.
...
- [ ] **Step 1: RED test extension** — standard/FIFO topic create 후 `findTopicArn`과 publish를 resolver 경로로 실행하고 create invalidation 경계를 확인한다.
- [ ] **Step 2: Floci sequential run**
...
- [ ] **Step 4: static/diff checks** — `./gradlew :bluetape4k-aws-spring-boot:detekt`가 존재하면 실행하고, `git diff --check`, changed Kotlin KDoc/API read-back, public constructor compatibility를 확인한다.

## Task 7: Type-A verifier, review, lesson, no-PR handoff

- [ ] **Step 1: spec/plan traceability** — 설계의 각 acceptance row를 구현 파일·테스트·명령 결과에 연결하고 gaps/risks를 기록한다.
- [ ] **Step 2: performance/stability scan** — cache lock 범위, clock expiry, flight cleanup, cancellation, allocation과 external emulator lifecycle을 점검한다. P0/P1이 있으면 Task 2–6으로 되돌린다.
- [ ] **Step 3: final checklist** — `verification-before-completion`으로 compile/test/static/diff/API/KDoc를 재검증한다.
- [ ] **Step 4: lesson** — `docs/lessons/2026-08-25-sns-topic-arn-resolver.md`를 Korean으로 작성하고 SPW-01~05를 통과시킨 뒤 Lore commit을 만든다. 구현 산출물과 테스트만 있고 durable lesson이 없다는 판단도 근거와 함께 기록한다.
- [ ] **Step 5: delivery boundary** — 현재 요청은 구현이며 PR repository/base/head와 PR 생성 권한을 명시하지 않았다. CG-11~CG-18/A-10~A-12는 N/A로 보고하고, PR/merge/remote push는 실행하지 않는다.
