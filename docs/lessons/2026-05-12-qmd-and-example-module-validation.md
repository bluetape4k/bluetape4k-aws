# qmd and example module validation
...
Use `qmd` first for existing design-plan discovery, then fall back to direct
file inspection when the current worktree is not indexed or when `qmd query`
tries to download a local reranking model.

## Outcome

`qmd` found the existing S3 design notes quickly for Spring Boot, but a broader
...
Prefer `qmd search` or `qmd query --no-rerank` for lightweight lookup during
implementation. Use `qmd get` for known artifacts. Avoid broad reranked queries
