...
- 검토 기준 데이터: `origin/develop` 기준 PR exact-head와 Task 10·hosted CI 수렴 변경
...
| Stability | PASS | 0/0 | 독립 검토가 renew·complete 일반 실패 후 `release()` 부재를 P1로 제시했으나, ambiguous mutation 뒤 stale claim을 release하지 않고 lease-expiry takeover에 맡기는 승인 설계와 fencing 계약에 반한다. sub-ms heartbeat는 validated 30초 이상 lease에서 불가능하고 store SPI는 non-blocking·cancellation-cooperative 구현을 요구한다. hosted CI가 기존 SQS FIFO handler의 `Mutex` 획득 경쟁으로 같은 그룹의 수신 순서가 뒤집힐 수 있음을 재현했다. submission 시점에 predecessor ticket을 연결해 같은 그룹은 순서를 지키고 다른 그룹은 병렬성을 유지하도록 수정했으며, 100회 반복과 전체 suite로 race·cleanup을 재검증했다. |
...
5. 최초 PR exact-head hosted CI에서 기존 `SqsMessageListenerContainerTest`의 FIFO 직렬화 검증이 1회 실패했다. 단순 재실행 대신 각 handler가 독립적으로 `Mutex`를 선점하던 원인을 확인하고, 같은 message group의 dispatch ticket을 등록 순서대로 연결했다. 해당 회귀 테스트를 100회 반복하도록 고정했다.
...
| SQS FIFO dispatch ordering | 회귀 테스트 100회, `SqsMessageListenerContainerTest` 116 tests, failure/error/skip 0 |
| 전체 `aws-spring-boot` + Floci | 177 test classes, 1,397 tests, failure/error 0, 기존 skip 2 |
...
최초 hosted exact-head run `33037601682`는 기존 FIFO test 1건이 실패했고 나머지
1,295 tests가 통과했다. 실패를 rerun-only로 처리하지 않고 dispatch ordering을 수정한
뒤 local 100회 반복과 전체 Floci suite를 통과했다. 새 exact-head hosted CI는 별도
PR gate에서 다시 확인한다.
