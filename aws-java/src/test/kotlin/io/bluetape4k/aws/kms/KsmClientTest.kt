...
class KsmClientTest: AbstractKmsTest() {

    companion object: KLogging()

    private val keyDescription = "예제용 KMS 키에 대한 설명입니다"
    ...
    private val aliasName = "alias/CustomAliasName"
    ...
    fun `Grant 생성`() {
        ...
        val response = client.createGrant(request)
        response.sdkHttpResponse().isSuccessful.shouldBeTrue()
    ...
    fun `커스텀 Alias 생성`() {
        ...
        val response = client.createAlias(request)
    ...
    fun `Alias 목록 조회`() {
        ...
        val response = client.listAliases(request)
    ...
    @Order(13)
    fun `Alias 삭제`() {
        val response = client.deleteAlias { it.aliasName(aliasName) }
        response.sdkHttpResponse().isSuccessful.shouldBeTrue()
    }
    ...
    fun `키 정책 설정`() {
        ...
        val response = client.putKeyPolicy(request)
        response.sdkHttpResponse().isSuccessful.shouldBeTrue()
    }
}
