...
class EventBridgeKtorTemplateTest {
    ...
    fun `putTargets applies configured default event bus when omitted`() = runTest {
        ...
        val result = template(client, defaultEventBusName = "orders")
    ...
    @Test
    fun `putEventsAsync returns SDK future without awaiting`() {
        val client = mockk<EventBridgeAsyncClient>()
        val response = PutEventsResponse.builder().failedEntryCount(0).build()
        val future = CompletableFuture.completedFuture(response)
        val entry = putEventsRequestEntryOf("orders", "order.created", "{}")

        every { client.putEvents(any<PutEventsRequest>()) } returns future

        val result = template(client).putEventsAsync(listOf(entry))

        result shouldBeSameInstanceAs future
        verify(exactly = 1) { client.putEvents(any<PutEventsRequest>()) }
    }

    private fun template(
        client: EventBridgeAsyncClient,
        defaultEventBusName: String? = null,
    ): EventBridgeKtorTemplate =
        EventBridgeKtorTemplate(client, defaultEventBusName)
}
