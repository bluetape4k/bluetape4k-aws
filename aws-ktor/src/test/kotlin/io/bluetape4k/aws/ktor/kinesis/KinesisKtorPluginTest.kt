...
class KinesisKtorPluginTest {
    ...
    fun `plugin stores injected operations`() = testApplication {
        application {
            ...
            install(KinesisKtorPlugin) {
                kinesisOperations = operations
            }
        }

        startApplication()

        application.kinesis() shouldBeSameInstanceAs operations
        application.kinesisOrNull() shouldBeSameInstanceAs operations
        application.attributes[KinesisKtorRuntimeKey].operations shouldBeSameInstanceAs operations
    }
    ...
    fun `runtime closes plugin owned client once`() = runSuspendIO {
        val runtime = KinesisKtorRuntime(
    ...
    fun `service customizer runs after shared customizer`() = runSuspendIO {
        ...
        val runtime = KinesisKtorPluginConfig().apply {
            ...
            AwsKtorDefaults(
                ...
                kinesisAsyncClientCustomizers = listOf(
                    AwsKtorKinesisAsyncClientCustomizer { order += "shared" }
                ),
            )
        )
        requireNotNull(runtime)
