...
/**
 * Creates an AWS Kotlin SDK [KinesisClient].
 *
 * ```kotlin
 ...
 * ```
 *
 * @param endpointUrl Kinesis service endpoint URL; when null, uses the default AWS endpoint
 ...
 * @param builder additional configuration for [KinesisClient.Config.Builder]
 * @return the configured [KinesisClient]
 */
...
/**
 * Creates a [KinesisClient], executes [block], and closes the client automatically.
 *
 * The SDK manages its internal HTTP engine, so closing the client also shuts down the engine.
 *
 * ```kotlin
 * withKinesisClient(endpointUrl, region, credentialsProvider) { client ->
 *     client.putRecord { ... }
 * }
 * ```
 *
