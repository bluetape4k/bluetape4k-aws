...
class ImdsCoroutinesTemplateTest {
    ...
    fun `get returns string metadata`() = runTest {
        ...
        val template = ImdsCoroutinesTemplate(client, ImdsProperties())
    ...
    fun `get is bounded by request timeout`() = runTest {
        ...
        val template = ImdsCoroutinesTemplate(
            client,
            ImdsProperties(requestTimeout = Duration.ofMillis(10)),
        )
